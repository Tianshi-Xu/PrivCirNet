...
def ILP(args,test_loader,model):
    ...
    prob += sum(variable[f"b1_{i}"]*latency_weights_b1[i] +variable[f"b2_{i}"]*latency_weights_b2[i] + variable[f"b4_{i}"]*latency_weights_b4[i] +variable[f"b8_{i}"]*latency_weights_b8[i] +variable[f"b16_{i}"]*latency_weights_b16[i] for i in range(num_variable))-origin_latency*1.8 <= 0.01
    ...
    _logger.info("current_latency-origin_latency:"+str(current_latency-origin_latency*1.8))
