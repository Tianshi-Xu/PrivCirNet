...
class PruneMobileNetV2(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1.0,prune_ratio=0):
        super(PruneMobileNetV2, self).__init__()
        block = PruneInvertedResidual
        input_channel = 32
        last_channel = 1280
        if input_size == 224:
...
def prune_mobilenet(n_class, input_size, width_mult,prune_ratio=0) -> PruneMobileNetV2:
    model = PruneMobileNetV2(n_class=n_class, input_size=input_size, width_mult=width_mult,prune_ratio=prune_ratio)
    return model
...
@register_model
def c100_prune_mobilenetv2(prune_ratio=0,**kwargs):
    model=prune_mobilenet(100,32,1,prune_ratio=prune_ratio)
    return model

@register_model
def c10_prune_mobilenetv2(prune_ratio=0,**kwargs):
    model=prune_mobilenet(10,32,1,prune_ratio=prune_ratio)
    return model
